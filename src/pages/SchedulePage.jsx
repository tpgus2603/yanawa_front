...
const SchedulePage = () => {
  ...
  // 마우스 누름으로 시작 (클릭/드래그 둘 다 처리)
  const handleSlotMouseDown = (timeIdx, e) => {
    if (!isEditMode) return;
  ...
  const handleSlotMouseEnter = (timeIdx) => {
    if (!isEditMode || !isMouseDownRef.current) return;
  ...
  const renderTimeSlot = (slotIndex) => {
    ...
    return (
        <div
            ...
            onMouseDown={(e) => handleSlotMouseDown(slotIndex, e)}
            onMouseEnter={() => handleSlotMouseEnter(slotIndex)}
            onMouseUp={() => handleSlotMouseUp()}
            // 기존 단일 클릭 로직을 유지하려면 아래를 사용 (중복 방지 필요)
            onDoubleClick={() => handleSlotClick(slotIndex)}
        >
